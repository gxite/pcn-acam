...
class Tracker():
    ...
    def update_tracker(self, detection_info, frame):
        ...
        area_list = []
        for bb in boxes:
            area_list.append(self.get_area(bb))
        boxes_area = np.array(area_list)

        #filters out low confidence scores and small bounding box area
        score_and_bbox_area = np.logical_and(scores > self.score_th, boxes_area>=BOUNDING_BOX_AREA_THRESHOLD)
        #filters out non-person detections
